...
**完成标准（Checkpoint）**：
- [x] 编译 + 测试通过（71 个用例，含 MQ 发送/消费集成测试）。
- [x] 审批完成能产生异步通知（审批通过/拒绝 → MQ → 消费者 → sys_message）。
- [x] Swagger 覆盖通知查询/已读接口（系统通知分组）。
- [x] 输出阶段报告，**停止等待确认**。

> 说明：通知表采用 `sys_message`（`message` 为 MySQL 保留字，且符合 `sys_` 前缀约定），非 dev.md 草拟的 `message` 表名。
...
**完成标准（Checkpoint）**：
- [x] 先提交接口设计文档，**等待确认后再编码**（prompt 要求）。
- [x] 实现后编译 + 测试通过（81 个用例，含上传/下载/列表/删除测试）。
- [x] 输出阶段报告，**停止等待确认**。

> 已确认决策：存储本地磁盘 `./uploads/`（`upload.dir` 配置）；文件全员共享（列表/下载对登录用户开放，删除限上传者）；删除=逻辑+物理；上传限 20MB + 类型白名单（图片/pdf/word/excel/ppt/zip/txt）。预留 `StorageService` 接口，后续可换 MinIO。
